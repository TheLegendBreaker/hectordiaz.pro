...
export default createStore({
	...
	actions: {
		async reqPosts ({state,commit}) {
			...
			try {
				const response = await fetch(`http://api.hectordiaz.pro/share/wp-json/wp/v2/posts?page=1&per_page=20&_embed`)
					.then(res => res.json());
		...
		async reqPost ({state,commit}, id) {
			const qString = `http://api.hectordiaz.pro/share/wp-json/wp/v2/posts?include[]=`+id+`&_embed`
